...
Additionally, many CA servers serve certificates over plain HTTP. When your AEV-PDF instance is hosted over HTTPS, browsers block these HTTP requests (mixed content policy). The CORS proxy resolves both issues by routing requests through an HTTPS endpoint with proper headers.
...
If you're self-hosting AEV-PDF, you'll need to deploy your own CORS proxy for digital signatures to work with certificates that require chain fetching.
...
3. **Clone AEV-PDF and update allowed origins**:

   ```bash
   git clone https://github.com/alam00000/AEV-PDF.git
   cd AEV-PDF/cloudflare
   ```
   ...
   ::: warning Important
   Without this change, the proxy will reject all requests from your site with a **403 Forbidden** error. The default only allows requests from `AEV-PDF.com`.
   :::
...
4. **Deploy the proxy**:
   ...
   Note your worker URL (e.g., `https://AEV-PDF-cors-proxy.your-subdomain.workers.dev`).

5. **Rebuild AEV-PDF with the proxy URL**:
