...
Host AEV-PDF on your own server using Nginx.
...
Create `/etc/nginx/sites-available/AEV-PDF`:
...
```bash
sudo ln -s /etc/nginx/sites-available/AEV-PDF /etc/nginx/sites-enabled/
sudo nginx -t
